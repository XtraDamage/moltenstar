...
export const GENERIC_AGENT = {
  id: 'generic',
  name: 'Assistant',
  role: 'AI',
  systemPrompt: 'You are a helpful AI assistant. Answer accurately and concisely.',
  isGeneric: true
};

export function buildMessagesForAgent(agent, allAgents, conversationMessages) {
  if (!agent) return [];
  ...
  for (const msg of conversationMessages) {
    if (msg.role === 'user') {
      ...
      if (msg.replyToAgentId) {
        const targetAgent = allAgents.find(a => a.id === msg.replyToAgentId);
        const targetName = targetAgent ? targetAgent.name : 'Unknown';
        content = `[Replying to ${targetName}]: ${content}`;
      }
      messages.push({ role: 'user', content });
    } else if (msg.role === 'assistant') {
      if (msg.agentId === agent.id || agent.isGeneric) {
