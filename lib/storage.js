...
export function getSettings() {
  if (typeof window === 'undefined') return { theme: 'auto', colorScheme: 'graphite' };
  try {
    const stored = localStorage.getItem('moltenstar_settings');
    return stored ? JSON.parse(stored) : { theme: 'auto', colorScheme: 'graphite' };
  } catch {
    return { theme: 'auto', colorScheme: 'graphite' };
  }
}
